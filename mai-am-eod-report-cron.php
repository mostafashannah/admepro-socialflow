...
$ams = $pdo->query("SELECT id, name, email, whatsapp_number FROM team_members WHERE role IN ('account_manager','account_director','account_executive') AND status = 'active' AND mai_checkins_enabled = 1 AND whatsapp_number IS NOT NULL AND whatsapp_number != ''")->fetchAll(PDO::FETCH_ASSOC);
$started = 0;
foreach ($ams as $am) {
    try {
