        ...
        [
            ...
            'input_schema' => [
                ...
                'properties' => [
                    ...
                    'summary'       => ['type' => 'string', 'description' => 'ONLY a short 2-3 sentence overview of what the interaction was about — never put the detailed breakdown, section-by-section notes, or per-topic feedback here. Anything more granular than a one-line-per-topic recap belongs in key_points instead, formatted as separate lines.'],
                    'key_points'    => ['type' => 'string', 'description' => 'The detailed discussion content goes HERE, not in summary — one point per line, newline-separated, no leading dashes/bullets/numbers (the app renders each line as its own bullet). If the debrief covers several topics/sections (e.g. different projects, content grids, campaigns), give each its own line starting with the topic name, e.g. "SLVR Content Plan: ...". Break a long section into multiple short lines rather than one run-on line.'],
                    'action_items'  => ['type' => 'string', 'description' => 'Bullet-style follow-up actions/tasks, one per line, newline-separated, no leading dashes — empty string if none mentioned'],
                ],
                'required' => ['summary'],
            ],
        ],
...
function hasRecruitmentPermission(PDO $pdo, string $senderRole) {
    ...
    return in_array($role, ['hr', 'account_director'], true);
}
